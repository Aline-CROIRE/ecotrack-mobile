...
import { MapPin, Phone, Calendar, ChevronLeft, User as UserIcon, Navigation as NavIcon, MessageCircle } from 'lucide-react-native';
...
const RequestDetailScreen = ({ route, navigation }) => {
  ...
  const openMaps = () => {
    if (!hasCoords) return Alert.alert('Error', 'No GPS data');
    ...
    <Container>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <Content>
          ...
          <InfoBox>
            <InfoItem><MapPin size={20} color="#15803d" /><Value>{item?.location?.address || "No Address"}</Value></InfoItem>
            <InfoItem><Calendar size={20} color="#15803d" /><Value>{new Date(item.scheduledDate).toDateString()}</Value></InfoItem>
          </InfoBox>
          ...
          <ContactCard>
            ...
            <View style={{ flexDirection: 'row', gap: 10 }}>
                {/* CALL BUTTON */}
                <IconBtn onPress={() => Linking.openURL(`tel:${isCollector ? item.citizen?.phone : item.collector?.phone}`)} bg="#15803d">
                    <Phone color="#fff" size={18} />
                </IconBtn>

                {/* CHAT BUTTON - Opens the Chat Screen */}
                <IconBtn onPress={() => navigation.navigate('Chat', { recipientName: isCollector ? item.citizen?.name : item.collector?.name })} bg="#0f172a">
            ...
            <ActionArea>
              {status === 'assigned' ? (
                ...
                <Btn bg="#16a34a" onPress={() => handleUpdate('completed')} disabled={loading}>
                  {loading ? <ActivityIndicator color="#fff" /> : <BtnText>Mark Finished</BtnText>}
                </Btn>
              )}
            </ActionArea>
          )}
...
const InfoBox = styled.View` background: #f8fafc; padding: 15px; border-radius: 15px; margin-bottom: 15px; border: 1px solid #e2e8f0; `;
...
const IconBtn = styled.TouchableOpacity` background: ${props => props.bg}; padding: 10px; border-radius: 10px; `;
